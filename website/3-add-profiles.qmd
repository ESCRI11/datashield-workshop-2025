...
1. **Access Opal**: Navigate to `http://localhost:8080` and login as administrator
...
4. **Test connectivity**: Each profile should show as "Available" in the DataSHIELD configurations list
...
1. **Start simple**: Begin with 2-3 profiles, add more as needed

2. **Consistent naming**: Use descriptive profile names
   - `default` for standard DataSHIELD packages
   - `genomics` for omics analysis
   - `survival` for survival analysis
   - `dev` for testing new packages

3. **Document your profiles**: Keep a simple list
   ```
...
4. **Test regularly**: After adding profiles, test connectivity from R

5. **Clean up unused profiles**: Remove profiles you're not using to save resources
